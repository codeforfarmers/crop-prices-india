 ...
 SCHEDULER.every '45m' do
    ...
    names.each do |names|  
        if names.text == "Malabar"
            rate = names.parent.parent.next_sibling.next_sibling.text.strip
            spices[q+=1] = { label: "BlackPepper", value: "Rs."+rate }
        end
        ...
        if names.text == "Peppergarbled"
            rate = names.parent.parent.next_sibling.next_sibling.text.strip
            spices[q+=1] = { label: "PepperGarbled", value: "Rs."+rate }
        end
        if names.text == "Pepperungarbled"
            rate = names.parent.parent.next_sibling.next_sibling.text.strip
	...
	if dry.empty? || fiber.empty? || forest.empty? || livestock.empty? || other.empty?
		puts "list dry/fiber/forest/livestock/other is empty"
	else
		puts "list dry/fiber/forest/livestock/other not empty"
	end
